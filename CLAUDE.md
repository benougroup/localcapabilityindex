...
Instructions for LLM coding agents working on Local Capability Index.
...
All current services and businesses are synthetic test data, as confirmed by the user. Focus on crawling, indexing, retrieval, relationship extraction, and source attribution. Interviews with real Hong Kong and Singapore businesses are a later phase managed by the user; do not redirect the current task toward interviews.

Synthetic discovery tests do not establish that an engine will recommend real providers. Do not invent further qualifications, reviews, treatment outcomes, or provider capabilities to improve discovery. Derive relationships from existing explicit links.
...
- After changes to discovery code, run `PYTHONDONTWRITEBYTECODE=1 python3 -m unittest discover -s tests`.
...
4. Regenerates all four directories and the sitemap.
5. Follows local HTML anchors starting at the homepage; fails on missing destinations or unreachable content.
...
Tests cover reachability, missing/orphan detection, sitemap coverage, and service/country mappings. Local validation does not check external business websites, prove provider claims, or demonstrate search-engine indexing.
